...
SELECT *
        FROM (
          ...
          FROM SPARC_RAW.RBOK_GA.GA4_EVENTS_RAW_SAMPLE
        ) AS data_to_pivot
        PIVOT (
          MAX(EVENT_PARAMS_VALUE) FOR EVENT_PARAMS_KEY IN ('content','anonymize_ip','page_owner','page_type','engaged_session_event','market','entrances','term','source','page_location','url','medium','campaign','ga_session_id','cm_mmc2','page_title','session_engaged','engagement_time_msec','pathname','ga_session_number','page_referrer','firebase_conversion','version_info','filters_applied','ignore_referrer')
        ) AS pivoted_data
        PIVOT (
          MAX(USER_PROPERTIES_VALUE) FOR USER_PROPERTIES_KEY IN ('user_id')
        ) AS pivoted_data_1