...
--We will now UNION ALL the aggregated tables together coming from GA4, FRS, MAO and Inventory
--Each of the UNIONED Section have couple of comments called 'Common', 'Retail', 'GA4', 'Inventory', 'MAO' that to points the beginning of all columns below that represent the respective data, Common states that all the columns below are shared among FRS, GA4, MAO and Inventory Aggregated tables
--Source are defined in each of the UNION section to distinguish between the source of all tables
--BUSINESS_PERFORMANCE_DATE_LOCATION_KEY: This key is defined for FRS, GA4 and MAO which will be used in Tableau to connect to Budget and Forecast files. Each of the keys are created with a different logic across FRS, GA4 and MAO
--PRODUCT_DETAIL_STYLE_MONTH_KEY: This key is defined for FRS, GA4 and MAO which will be used in Tableau
--SALES_CHANNEL: Created to identify 'Reebok', 'Marketplace' and 'Retail Sales' for FRS, GA4, MAO and Inventory
--GA_MEDIA_CHANNEL_MONTH_KEY: This key is defined for GA4 only to connect to Budget and Forecast files in Tableau
--GA_MEDIA_CHANNEL_DATE_KEY: This key is defined for GA4 only to connect to Budget and Forecast files in Tableau
Select * from (
SELECT
--Common
...
NULL AS ENGAGED_GA_USER_SESSION_KEY,
NULL AS LASTTOUCH_TRAFFIC_CAMPAIGN,
NULL AS LASTTOUCH_TRAFFIC_MEDIUM,
NULL AS LASTTOUCH_TRAFFIC_SOURCE,
NULL AS MEDIA_CHANNEL,
NULL AS RBK_MEDIA_CHANNEL,
NULL AS GA_MEDIA_CHANNEL_MONTH_KEY,
NULL AS GA_MEDIA_CHANNEL_DATE_KEY,
NULL AS RBK_MEDIA_CHANNEL_FISCALMONTH_KEY,
NULL AS RBK_MEDIA_CHANNEL_DATE_KEY,
NULL AS FUNNEL,
NULL AS RBK_FUNNEL,
--Inventory
...
NULL AS NET_DEMAND_DOLLARS,
NULL AS REPORTABLE_ORDER_ID,
...
CONCAT(DATEDIFF(day, '1900-01-01'::TIMESTAMP, EVENT_DATE)+2,'|',MEDIA_CHANNEL) as GA_MEDIA_CHANNEL_DATE_KEY,
CONCAT(ACCTGPRDMNTHNUM,'|',RBK_MEDIA_CHANNEL) as RBK_MEDIA_CHANNEL_FISCALMONTH_KEY,
CONCAT(DATEDIFF(day, '1900-01-01'::TIMESTAMP, EVENT_DATE)+2,'|',RBK_MEDIA_CHANNEL) as RBK_MEDIA_CHANNEL_DATE_KEY,
FUNNEL,
RBK_FUNNEL,
--Inventory
NULL AS ON_HAND_UNITS,
...
--Common
SOURCE as SOURCE,
CONCAT(DATEDIFF(day, '1900-01-01'::TIMESTAMP, DEMAND_DATE_LOCAL)+2,'|', BRAND_LOCATION) AS BUSINESS_PERFORMANCE_DATE_LOCATION_KEY,
CONCAT(ACCTGPRDMNTHNUM,'|',KEY_CATEGORY_DESC) AS PRODUCT_DETAIL_STYLE_MONTH_KEY,
